...
import 'package:dopablocker_mobile/core/channels/blocking_methods.dart';
...
///
/// Os nomes dos métodos ficam centralizados em [BlockingMethods] (espelham o
/// `BlockingMethods` do Kotlin).
abstract final class BlockingChannel {
  ...
  static Future<bool> startVpn() async {
    final ok = await _channel.invokeMethod<bool>(BlockingMethods.startVpn);
    return ok ?? false;
  }

  /// Para o serviço de VPN.
  static Future<void> stopVpn() => _channel.invokeMethod(BlockingMethods.stopVpn);

  /// Indica se a VPN está rodando.
  static Future<bool> isVpnActive() async {
    final active = await _channel.invokeMethod<bool>(BlockingMethods.isVpnActive);
    return active ?? false;
  }
  ...
  static Future<bool> isVpnPrepared() async {
    final prepared = await _channel.invokeMethod<bool>(BlockingMethods.isVpnPrepared);
  ...
  static Future<void> updateBlocklist(List<String> domains) =>
      _channel.invokeMethod(BlockingMethods.updateBlocklist, {'domains': domains});
  ...
  static Future<void> updateBlockedApps(List<String> packages) =>
      _channel.invokeMethod(BlockingMethods.updateBlockedApps, {'packages': packages});
  ...
  static Future<void> setAdultFilter(bool enabled) =>
      _channel.invokeMethod(BlockingMethods.setAdultFilter, {'enabled': enabled});
  ...
  static Future<void> setTamperConfig({
    ...
    required bool isChild,
      ...
      _channel.invokeMethod(BlockingMethods.setTamperConfig, {
  ...
  static Future<List<InstalledApp>> getInstalledApps() async {
    final raw = await _channel.invokeMethod<List<dynamic>>(BlockingMethods.getInstalledApps);
  ...
  static Future<bool> isAccessibilityEnabled() async {
    final enabled = await _channel.invokeMethod<bool>(BlockingMethods.isAccessibilityEnabled);
  ...
  static Future<void> openAccessibilitySettings() =>
      _channel.invokeMethod(BlockingMethods.openAccessibilitySettings);
  ...
  static Future<bool> canDrawOverlays() async {
    final ok = await _channel.invokeMethod<bool>(BlockingMethods.canDrawOverlays);
    return ok ?? false;
  }

  /// Abre a tela do sistema para o usuário conceder "Sobrepor a outros apps".
  static Future<void> requestOverlayPermission() =>
      _channel.invokeMethod(BlockingMethods.requestOverlayPermission);
}

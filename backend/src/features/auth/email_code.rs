// =============================================================================
// email_code — códigos e tokens de verificação de email.
// =============================================================================
// Fluxo de verificação por código que precede o `POST /auth/register` para o
// provider `password`: gera/valida o código de 6 dígitos (HMAC-SHA256), aplica
// cooldown/rate-limit e emite o `email_verification_token`. A entrega do código
// (SMTP real ou log de dev) fica em `email_delivery`.
// =============================================================================
...
use crate::core::models::{EmailCodeStartResponse, EmailCodeVerifyResponse};
use crate::core::util::{iso_after, iso_before, iso_now, map_sqlite_error, ServiceError};

use super::email_delivery::{log_verification_code, resolve_smtp_delivery, send_verification_email};
...
pub(crate) async fn expire_email_verification(db: &Connection, id: String) {
    let _ = db
        .call(move |c| {
            c.execute(
                "UPDATE email_verifications SET status = 'expired' WHERE id = ?1",
